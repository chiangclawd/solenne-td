...
#### 坑 J3：所有 multi-path 敵人都跑到 path 0（修了 J2 還是壞的）

- **情境**：v2.2.4 修了 WaveManager per-path streams、v2.6.2 強化 wave 資料每路 ≥3 敵，但**玩家實測仍只看到 path 0 有敵人**。
- **根因**：`GameScene.buildWave()` 把 JSON wave entry 重組成 runtime entry 時，**只複製 `delay` 與 `enemy`，把 `path` 欄位整個丟掉**。所以 WaveManager 收到的 entries 全部 `path === undefined`，分流時 fallback 到 path 0。
  ```ts
  // 壞的：path 欄位掉了
  ...
  ```
- **解法 (v2.8.1)**：一行修補 — `buildWave` 必須 forward `entry.path`。
- **教訓**：**在資料管線的每一個 reshape 點都要驗證所有欄位都帶過去**，不能只驗證 schema。資料 audit script 看的是 JSON 源頭、引擎 audit 看的是 runtime 行為，但中間 transformation 層（buildWave）是 dark spot — 沒驗證過。修 J2 時資料端 + 引擎端都對，中間層卻是壞的，三層才能組成完整正確流程。
- **教訓 2**：未來新增 LevelData 欄位時，要追蹤每個經手的 `.map()` / `{...spread}` / 物件重建，確保新欄位都被帶下去。一個方法是用 spread `return { ...entry, enemy: modified }` 而不是顯式列欄位 — spread 不會掉。
...
| v2.8.0 | C2 Codex lore 擴充 + 英雄頁籤 | 把所有 lore 文案集中到 `src/game/Lore.ts`；CJK-aware text wrapper；taller cards (124→152, 96→130)；新增 `heroes` tab 顯示 3 位指揮官背景 |
| v2.8.1 | hotfix：buildWave 把 path 欄位掉了，所有多路徑關卡實際上都是單路徑 | data ✅ + 引擎 ✅，但中間 transformation 層偷偷剝欄位 — 驗證資料管線時要看「所有 reshape 點」 |
